...
const Video: React.FC<VideoProps> = ({ width, height, videoRef, isCameraOn, isMicOn, nickName, userName }) => {
    return <div className={`relative flex h-full w-full aspect-w-16 aspect-h-9 bg-black rounded-2xl`}>
        <video ref={videoRef} autoPlay className={`object-cover rounded-2xl`}></video>
        {/* 카메라 on일 때 name tag */}
